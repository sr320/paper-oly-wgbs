...
```{r}
dml05251200plus <- filter(myDiff25p, strand == "+") %>% mutate(start = start -1, end = end + 1) %>% select(chr, start, end, strand, meth.diff)
dml05251200minus <- filter(myDiff25p, strand == "-") %>% mutate(start = start -2) %>% select(chr, start, end, strand, meth.diff)
...
```{r}
filtered.myobj=filterByCoverage(myobj.tiled,lo.count=NULL,lo.perc=NULL,
                                      hi.count=NULL,hi.perc=99.9)
