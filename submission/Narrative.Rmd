...
Of the `r nrow(cg)` loci in the draft genome `r nrow(cov)` (`r (nrow(cov)/nrow(cg))*100`%) had corresponding mapped reads, with `r nrow(cov5)` loci covered at 5x and `r nrow(cov10)` loci covered at 10x. When considering loci with 5x coverage `r ((188345)/nrow(meth))*100`% methylated loci overlapped with predicted genes while `r ((242856)/nrow(lessmeth))*100`% of less methylated loci overlapped with predicted genes.
