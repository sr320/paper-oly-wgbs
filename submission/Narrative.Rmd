...
Bedgraph used to describe the location of methylation
...
```{r}
cov2 <- mutate(cov, cvr = V5 + V6)
```

```{r}
ggplot(cov2, aes(x=cvr)) +
...
```{bash}
wc -l ../data_21/combined_10x.bedgraph
```

Of the `nrow(gm)` loci in the draft genome x had corresponding reads map with x loci covered at 5x
