...
Genomic DNA was subjected to bisulfite conversion using the EZ DNA Methylation-Gold Kit (ZymoResearch) according to the manufacturer's protocol, with the following changes. Used 100ng of DNA, centrifugation steps were performed at 13,000g, and desulphonation incubation period was 20 minutes.
...
Sequencing was performed by Genewiz, Inc. in a single flowcell on a HiSeq2500 (Illumina) with 50bp single-end reads. Raw sequence reads were deposited in the NCBI Short Read Archive (NCBI SRA: SRP163248). Read quality was evaluated with FastQC (v0.11.7; https://www.bioinformatics.babraham.ac.uk/projects/fastqc/) and visualized with MultiQC (v1.6.dev0;http://multiqc.info/).



## _Draft Genome Assembly_

A draft genome was generated in order to map bisulfite sequence reads. The draft genome for the Olympia oyster was created using a combination of short read sequence data (Illumina HiSeq4000) combined with long read sequence data (PacBio RSII) using PBJelly (PBSuite_15.8.24; English et al, 2012).
...
In preparation for bisufite mapping using Bismark (v0.19.0; https://github.com/FelixKrueger/Bismark), 10bp from the 5' end of each read were trimmed using TrimGalore! (v0.4.5; https://www.bioinformatics.babraham.ac.uk/projects/trim_galore/). Post-trimming read quality was assessed with FastQC (v0.11.7; https://www.bioinformatics.babraham.ac.uk/projects/fastqc/) and visualized with MultiQC (v1.6.dev0;http://multiqc.info/).
...
Identification of differentially methylated loci (DML) using R (v3.4.4; R Studio v1.1.383) and the MethylKit R package (v1.4.1; Akalin et al, 2012), with a minimum of 3x coverage. All code and analyses is available in the following GitHub repository: URL_NEEDED.
...
Genome assembly data were compiled using QUAST (v4.5; Gurevich et al, 2013). The assembly yielded 213,264 contigs, comprised of 1,180,563,613bp with an N50 of 12,433. The assembly contained 894 contigs >= 50,000bp. The largest contig was 194,507bp (Table NUMBER).

Bisulfite treated DNA sequencing resulted in 11,954,873 average reads per library..
